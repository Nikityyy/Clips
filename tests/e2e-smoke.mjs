...
try {
  ...
  const figureControlSizes = await page.locator('.composer-inline-control').first().evaluate((element) => [...element.children].map((child) => { const { width, height } = child.getBoundingClientRect(); return { width, height }; }));
  assert.equal(figureControlSizes[1].height, figureControlSizes[0].height, 'the add-character button should match the figure selector height');
  assert.equal(figureControlSizes[1].width, figureControlSizes[1].height, 'the add-character button should be square');
